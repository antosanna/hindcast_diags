...
ic=0
cd $datamm
plist=`ls | grep ${SPSsystem}_${yy}$st | cut -d '.' -f2 | cut -d '_' -f2-3`

for sps in $plist ; do

	
	cdo sub $datamm/${SPSsystem}_${sps}_${var}.zip.nc  $datamm/clim/${var}_${SPSSYS}_clim_$refperiod.${st}.nc $datamm/anom/${var}_${SPSSYS}_${sps}_ano.$refperiod.nc
	ic=`expr $ic + 1`
